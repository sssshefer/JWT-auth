...
export class MailDomainServiceImpl  {
    ...
    async sendActivationMail(to: string, link: string) {
        await this.transporter.sendMail({
            to,
            from: `JWT <${process.env.SMTP_USER}>`,
            ...
            html:
                `
                    ...
                    color:#1d1d1d;">You received this message because you created a new account
                     on ${process.env.CLIENT_URL} website. It is created to observe features of JWT authorization</h3>
                        <h3 style=" font-family: 'Montserrat', sans-serif, Arial;
                    color:#1d1d1d;">Please proceed to the following link to confirm your email address:</h3>
                        <a href="${link}">${link}</a>
                        <h3  style=" font-family: 'Montserrat', sans-serif, Arial;
                    color:#1d1d1d;">Best regards, <br/> Arkadii Shefer's JWT authorization project</h3>   
                    </div>
                `
        })
    }
    ...
    async sendNewPasswordMail(to: string, password: string) {
        await this.transporter.sendMail({
            ...
            html: `
                    <div>
                        <h3 style=" font-family: 'Montserrat', sans-serif, Arial;
                    color:#1d1d1d;">You received this message because you require 
                    a new password for your account on ${process.env.CLIENT_URL} website.
                     The website is created to observe features of JWT authorization</h3>
                        <h3 style=" font-family: 'Montserrat', sans-serif, Arial;
    ...
    async sendPasswordIsChangedMail(to: string) {
        await this.transporter.sendMail({
            ...
            html: `
                    <div>
                        <h3 style=" font-family: 'Montserrat', sans-serif, Arial;
                    color:#1d1d1d;">You received this message because the password 
                    for your account on ${process.env.CLIENT_URL} website has been changed</h3>
                        <h3 style=" font-family: 'Montserrat', sans-serif, Arial;
